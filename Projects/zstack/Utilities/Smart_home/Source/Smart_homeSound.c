/*******************************************************************
  ...
  作 者： 柳成林
  功 能： 声音检测功能节点，实现对声音响度的检测，收集声音震动信
          息并发送给传感器
  更新日志(2021)
  3.22
  + 新增 设置输入输出簇
...
{
  Smart_home_CLUSTERID_SOUNDVBMSG         //继电器接收的消息控制命令
};

const cId_t Smart_home_ClusterList_OUT[1] =
{
  Smart_home_CLUSTERID_SOUNDVBMSG         //继电器发送的状态信息
};
...
byte Coordinator_Msg[SOUNDVBMSG_LEN];
...
// 声音振动报警标志, soundVbBuf[0]:声音报警；soundVbBuf[1]:振动报警；
static uint8 soundVbBuf[2];
...
static void Smart_home_Send(void)
{
  ...
  do{
    ...
    // 第0位存储声音报警信息，第1位存储振动报警信息
    Coordinator_Msg[4] = soundVbBuf[1]<<1 | soundVbBuf[0];
