...
{
  Smart_home_CLUSTERID_HUMITMSG        //继电器接收的消息控制命令
};

const cId_t Smart_home_ClusterList_OUT[1] =
{
  Smart_home_CLUSTERID_HUMITMSG         //继电器发送的状态信息
};
...
static void Smart_home_Send(void)
{
  ...
  do{
    ...
    // 发送给协调器命令 
    Coordinator_Msg[4] = Sendbuf[0];  //温度
    Coordinator_Msg[5] = Sendbuf[1];  //湿度
    
    tmp = AF_DataRequest( &Coordinator_DstAddr,                         
                          (endPointDesc_t *)&Smart_home_epDesc,                  
                           Smart_home_CLUSTERID_HUMITMSG,
                           HUMITMSG_LEN,                 
                           Coordinator_Msg,                    
                          &Smart_home_MsgID,                       
                           AF_DISCV_ROUTE,                     
