/*******************************************************************
  ...
  作 者： 柳成林
  功 能： 电机功能节点，实现向网关节点的发送以及接收网关节点的消息
  更新日志(2021)
  3.21
  + 新增 设置输入输出簇
...
{
  Smart_home_CLUSTERID_MOTORCTRL         //继电器接收的消息控制命令
};

const cId_t Smart_home_ClusterList_OUT[1] =
{
  Smart_home_CLUSTERID_MOTORSTATUSMSG         //继电器发送的状态信息
};
...
{
    static uint8 motorStatusCnt;
    
    if ( keys & HAL_KEY_SW_5 )
    {
      motorStatusCnt++;
      motorStatusCnt = motorStatusCnt % 8;
      switch(motorStatusCnt)
      {
        case 0:  
          HalMotorCtl(HAL_MOTOR_STOP, 0);       // 电机停止
          break;
        case 1: 
          HalMotorCtl(HAL_MOTOR_FORWORD, 10);   // 电机正转，转速10转/秒
          break;
        case 2: 
          HalMotorCtl(HAL_MOTOR_FORWORD, 30);   // 电机正转，转速30转/秒
          break;
        case 3: 
          HalMotorCtl(HAL_MOTOR_FORWORD, 50);   // 电机正转，转速50转/秒
          break;
        case 4: 
          HalMotorCtl(HAL_MOTOR_STOP, 0);       // 电机停止
          break;
        case 5: 
          HalMotorCtl(HAL_MOTOR_BACKWORD, 10);  // 电机反转，转速10转/秒
          break;
        case 6: 
          HalMotorCtl(HAL_MOTOR_BACKWORD, 30);  // 电机反转，转速30转/分钟
          break;
        case 7: 
          HalMotorCtl(HAL_MOTOR_BACKWORD, 50);  // 电机反转，转速50转/分钟
          break;
        default:
          break;
      }
...
void Smart_home_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )
{
  ...
  switch ( pkt->clusterId )
  {
    ...
    {
      // 收到的电机控制命令
      speed = pkt->cmd.Data[4];
      motorCtl = pkt->cmd.Data[5];
            
     // 测试收到的命令       
#if (HAL_UART == TRUE)
      HalUARTWrite(HAL_UART_PORT_0, & speed,  1);
      HalUARTWrite(HAL_UART_PORT_0, & motorCtl,  1);
#endif             
      HalMotorCtl(motorCtl, speed);       
      break;
    } 
...
static void Smart_home_Send(void)
{
  ...
  do{
    ...
    // 发送给协调器命令 
    Coordinator_Msg[4] = HalMotorSpeedCheck();
    Coordinator_Msg[5] = HalMotorStatusCheck();
    
    tmp = AF_DataRequest( &Coordinator_DstAddr,                         
                          (endPointDesc_t *)&Smart_home_epDesc,                  
                           Smart_home_CLUSTERID_MOTORSTATUSMSG,
                           MOTORSTATUSMSG_LEN,                 
                           Coordinator_Msg,                    
                          &Smart_home_MsgID,                       
                           AF_DISCV_ROUTE,                     
