/*******************************************************************
  文件名：Smart_homeTempLight.c
  作 者： 柳成林
  功 能： 光照检测功能节点，实现对光照以及温度的检测，收集光照温度信
          息并发送给传感器
  更新日志(2021)
  3.22
  + 新增 设置输入输出簇
...
{
  Smart_home_CLUSTERID_TEMPLIGHTMSG         //继电器发送的状态信息
};
...
static void Smart_home_Send(void)
{
  uint8 tmp;
  uint16 Light,Temper;
  static uint8 Sendbuf[4];
  
  
  Light = BH1750_ConvertLight();
  Temper = DS18B20_ConvertTemp();
  
  Sendbuf[0] = (uint8)((Temper>>4) & 0x007F);  // 温度的整数部分
  Sendbuf[1] = (uint8)(Temper & 0x000F);       // 温度的小数部分
  Sendbuf[2] = (uint8)(Light >> 8);            // 光照度的高8位
  Sendbuf[3] = (uint8)(Light & 0xFF);          // 光照度的低8位
  ...
  do{
    ...
    // 发送给协调器命令 
    Coordinator_Msg[4] = Sendbuf[0];
    Coordinator_Msg[5] = Sendbuf[1];
    Coordinator_Msg[6] = Sendbuf[2];
    Coordinator_Msg[7] = Sendbuf[3];
    
    tmp = AF_DataRequest( &Coordinator_DstAddr,                         
                          (endPointDesc_t *)&Smart_home_epDesc,                  
                           Smart_home_CLUSTERID_TEMPLIGHTMSG,
                           TEMPLIGHTMSG_LEN,                 
                           Coordinator_Msg,                    
                          &Smart_home_MsgID,                       
                           AF_DISCV_ROUTE,                     
