/*******************************************************************
  文件名：Smart_homeD203.c
  作 者： 柳成林
  功 能： 红外释电功能节点，实现对人员的检测，收集红外信
          息并发送给传感器
  更新日志(2021)
  3.24
  + 新增 设置输入输出簇
...
{
  Smart_home_CLUSTERID_INFRAREDMSG        //继电器接收的消息控制命令
};

const cId_t Smart_home_ClusterList_OUT[1] =
{
  Smart_home_CLUSTERID_INFRAREDMSG         //继电器发送的状态信息
};
...
static void Smart_home_CHECK_EVT(void)
{
  ...
  if (infraredAlertCnt > ALARM_MAX_CNT)
  {
    InfraredData = 1;
    infraredAlertCnt = 0;
  }
