...
void ZDO_StartDevice( byte logicalType, devStartModes_t startMode, byte beaconOrder, byte superframeOrder )
{
  ...
  if ( ZG_BUILD_COORDINATOR_TYPE && logicalType == NODETYPE_COORDINATOR )
  {
    ...
    else
    {
      ...
      HalUARTWrite(HAL_UART_PORT_0, "StartDevice ERR\nMODE unknown\n", 
                   strlen("StartDevice ERR\nMODE unknown\n"));
#endif
    }
  }
...
#if defined(HAL_UART_TEST)
  HalUARTWrite(HAL_UART_PORT_0, "End Device Bind\n", strlen("End Device Bind\n"));
  if ( Status == ZDP_SUCCESS )
  {
    HalUARTWrite(HAL_UART_PORT_0, "Success Sent\n", strlen("Success Sent\n"));
  }
  else
  {
    HalUARTWrite(HAL_UART_PORT_0, "Timeout\n", strlen("Timeout\n"));
    HalLcdWriteString( "Timeout", HAL_LCD_LINE_2 );
  }
#endif  
...
#if defined(HAL_UART_TEST)
      HalUARTWrite(HAL_UART_PORT_0, "Match Desc Req\nRsp Sent\n", 
                   strlen("Match Desc Req\nRsp Sent\n"));
#endif
    }
  }
  else
