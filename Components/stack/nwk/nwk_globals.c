 ...
 * STATUS STRINGS
 */
#if ( LCD_SUPPORTED || HAL_UART_TEST )
...
void nwk_Status( uint16 statusCode, uint16 statusValue )
{
  ...
  switch ( statusCode )
  {
    ...
    case NWK_STATUS_COORD_ADDR:
      if ( ZSTACK_ROUTER_BUILD )
      {
        strcpy(buf, ZigbeeCoordStr);
        strcat(buf, "\n");
        strcat(buf, NetworkIDStr);
        strcat(buf, " ");
        _ltoa( statusValue, (uint8 *)&buf[strlen(buf)], 16 );
        strcat(buf, "\n");
    ...
    case NWK_STATUS_ASSOC_CNF:
        strcpy(buf, AssocCnfStr);
        strcat(buf, SuccessStr);
        strcat(buf, "\n");
        HalUARTWrite(HAL_UART_PORT_0, (uint8 *)buf, strlen(buf));
      break;

    case NWK_ERROR_ASSOC_CNF_DENIED:
        strcpy(buf, AssocCnfFailStr);
        strcat(buf, ": ");
        _ltoa( statusValue, (uint8 *)&buf[strlen(buf)], 16 );
        strcat(buf, "\n");
        HalUARTWrite(HAL_UART_PORT_0, (uint8 *)buf, strlen(buf));
    ...
    case NWK_ERROR_ENERGY_SCAN_FAILED:
        strcpy(buf, EnergyLevelStr);
        strcat(buf, ScanFailedStr);
        strcat(buf, "\n");
        HalUARTWrite(HAL_UART_PORT_0, (uint8 *)buf, strlen(buf));
      break;
  }
